...
const graduateList: GraduateItem[] = [
    ...
    { id: 8, column: '待遇・\n福利厚生', detail: '社会保険完備（正社員登用後）、交通費支給、書籍購入補助' },
...
export default function Graduate() {
    return (
        <div className="border w-full">
            <h1 className="text-center my-16 font-bold lg:text-2xl text-xl mx-8 md:mx-auto">
                未経験からITエンジニアを目指す方
            </h1>
            <div className='text-center my-4 mx-8 lg:mx-auto'>
                <p className='font-bold text-lg lg:text-xl my-4 lg:mx-auto'>IT業界への一歩を、Hiyokuで踏み出しませんか？</p>
                <p className='text-sm lg:text-base'>
                    「IT業界に興味はあるけれど、経験がなくて不安…」
                    <br/>そんなあなたをHiyokuは全力でサポートします！
                    <br />ITへの熱い想いと学ぶ意欲さえあれば、経験は一切問いません。
                </p>
            </div>
                    ...
                    <tbody className="border-t">
                        {graduateList.map((item) => (
                            <tr key={item.id} className="h-16 border-b">
                                <th className="font-thin text-base w-24 md:w-32 lg:w-64 md:px-4 text-xs sm:text-base lg:text-base whitespace-pre-line">
                                    {item.column}
                                </th>
                                <td className="py-4 md:px-4 whitespace-pre-line text-xs sm:text-sm lg:text-base">
                                    {item.detail}
                                </td>
                            </tr>
                        ))}
